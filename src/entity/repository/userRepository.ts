...
class UserRepository implements IRepository<User> {
  ...
  private readonly logger = Logger('userLogger')
  ...
  public async instantiate(data: Object): Promise<User | undefined> {
    try {
      ...
      this.logger.info('User instance created')
      return user
    } catch (error) {
  ...
  public async update(
    ...
    this.logger.info('Updating user by email: ', email)
    try {
  ...
  public async delete(
    ...
    this.logger.info('Deleting user by email: ', email)
    try {
      const user = await this.userRepository.findOneOrFail(email)
      if (user) {
